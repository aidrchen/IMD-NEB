...
EXTERN real neb_k INIT(0.0001);                /* spring constant */
EXTERN int smart_climb INIT(0);
EXTERN int neb_climb[10] INIT (zero10);			/* climbing image */
EXTERN int neb_slide[10] INIT (zero10);			/* sliding image */
EXTERN real neb_fnorm INIT(0.0);            /* total force norm */
EXTERN real neb_delta INIT(-1.0);		/* convergence parameter */
EXTERN real old_totpotenergy INIT(0.0);	/* tot_eng of last step*/
EXTERN int neb_stop INIT(0);			/* convergence flag */
EXTERN char *neb_outfilename INIT(NULL);    /* name of NEB .eng file */
EXTERN FILE *neb_eng_file INIT(NULL);       /* pointer to NEB .eng file */
EXTERN int neb_fe INIT(-1);				/* free end iamge */
EXTERN int neb_ke_step INIT(0);			/* spring k estimating */
EXTERN int neb_ofe INIT(-1);
EXTERN real norm1 INIT(0.0);
EXTERN real norm2 INIT(0.0);
EXTERN real checkangle INIT(0.0);
EXTERN real checkangle2 INIT(0.0);
#endif
